...
public class FrameMovieReview extends JPanel {
	public FrameMovieReview() {
		...
		//평점 받기(radio)
		JPanel dis1=new JPanel(); //radio, 한줄평을 받을 패널 생성
		...
		for(int i=0; i<mName.length; i++) {
			...
			movie[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					//어떤 radio든 선택하는 순간 한줄평 작성 폼이 화면에 보여진다.
					dis1.setVisible(true);
					
					String[] movieurl={"라라랜드.png", "말할수없는비밀.jpg", "인사이드아웃.png", 
				...
				if(point1.isSelected() || point2.isSelected() || 
					...
					for(int i=0; i<movie.length; i++) {
					    if(movie[i].isSelected()) {
					    	//작성한 후기를 영화 리스트에 추가
					    	new MovieDAO().inputReview(movie[i].getText(), 
					    			reviewName.getText(), 
					    			review.getText(), num);
				        }
